...
func (s *Server) handleUpdateRegistry(w http.ResponseWriter, r *http.Request) {
	...
	if req.OriginalName == "" || req.Name == "" || req.URL == "" {
		http.Error(w, "OriginalName, Name and URL are required", http.StatusBadRequest)
		return
	}

	if err := manager.UpdateRegistry(req.OriginalName, req.Name, req.URL); err != nil {
