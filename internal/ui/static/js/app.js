...
async function loadData() {
    try {
        ...
        if (list) list.innerHTML = `<span class="error">Error loading data: ${e}</span>`;
    }
}
            ...
            <li style="margin-bottom: 0.5rem;">
                <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                    ...
                    <div style="flex-shrink: 0;">
                        <button class="outline small" style="width: auto; padding: 0.25rem 0.5rem; margin-right: 0.5rem;" onclick="testServer('${name}')" id="test-btn-${name}" title="Test connection to server">⚡ Test</button>
                        <button class="outline small" style="width: auto; padding: 0.25rem 0.5rem; margin-right: 0.5rem;" onclick="inspectServer('${name}')" title="View the command to run this server manually">🔍 Inspect</button>
    ...
    for (const [name, client] of Object.entries(configData.clients)) {
        html += `
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem; border-bottom: 1px solid var(--muted-border-color); padding-bottom: 0.25rem;">
                ...
                <button class="outline secondary small" style="width: auto; padding: 0.2rem 0.5rem;" onclick="inspectClientConfig('${name}')" title="View Config File">👁️</button>
            </div>
        `;
    }
    list.innerHTML = html;
...
async function addRegistry() {
    ...
    try {
        ...
        if (res.ok) {
            ...
            loadData(); // Reload to show new registry
        } else {
...
async function testServer(serverID) {
    ...
    try {
        ...
        if (result.success) {
            showToast(`✅ Connected! Latency: ${result.latency/1000000}ms. Version: ${result.version || 'Unknown'}`, 'success');
        } else {
...
async function applyConfig() {
    ...
    try {
        ...
        for (const res of data.results) {
            if (res.success) {
                html += `<li class="success">✅ ${res.clientName}: Success (Backup: ${res.backupPath})</li>`;
            } else {
                html += `<li class="error">❌ ${res.clientName}: Failed - ${res.error}</li>`;
            }
        }
        html += '</ul>';
...
async function searchServers() {
    ...
    try {
        ...
        for (const s of servers) {
            ...
            let toolsBadge = s.Tools > 0 ? `<small style="background:var(--primary); color:var(--primary-inverse); padding:2px 6px; border-radius:4px; margin-right:4px;">🛠️ ${s.Tools}</small>` : '';
...
async function loadBackups() {
    try {
        ...
        console.error(e);
        const list = document.getElementById('backupsList');
        if (list) list.innerHTML = `<span class="error">Error loading backups: ${e}</span>`;
    }
}
...
function renderKnownClients(clients) {
    ...
    clients.forEach(c => {
        ...
        const statusBadge = isConfigured
            ? `<span style="color:var(--ins-color)">✅ Detected</span>`
            : `<span style="color:var(--del-color)">❌ Not Detected</span>`;
...
async function loadLogs() {
    const container = document.getElementById('logsContainer');
    // Check if element exists (tab might not be active or loaded yet, though normally it is)
    if (!container) return;
...
async function checkDoctor() {
    try {
        ...
        results.forEach(r => {
            ...
            if (r.status === 'error') { icon = '❌'; color = 'var(--del-color)'; }
            else if (r.status === 'warning') { icon = '⚠️'; color = 'var(--pico-color-amber-500)'; }
